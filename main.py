...
def encrypt(input_bytes, key):
    """Function encrypts the input_bytes according to AES(256) algorithm using the key
    Args:
       input_bytes -- list of int less than 255, ie list of bytes. Length of input_bytes is constantly 16
       key -- a strig of plain text. The same string is used in decryption
    Returns:
        List of int
    """
    ...
    state = [[] for j in range(8)]
    for r in range(8):
        for c in range(nb):
            state[r].append(input_bytes[r + 8 * c])
    ...
    output = [None for i in range(8 * nb)]
    for r in range(8):
        for c in range(nb):
            output[r + 8 * c] = state[r][c]

    return output


def decrypt(cipher, key):
    """Function decrypts the cipher according to AES(256) algorithm using the key
    Args:
...
def key_expansion(key):
    """It makes list of RoundKeys for function AddRoundKey.
    """
...
def mul_by_03(num):
    # The function multiplies by 3 in Galua space. Addition in Galua field is oparetion XOR
